...
class Student { // Student class banayi hai
  name: string; // name property define ki
  constructor(n: string) { // constructor method banaya
    this.name = n; // n ko name property me assign kiya
  }
}

class Person { // Person class banayi hai
  students: Student[] = []; // students property ko initialize kiya as an empty array
  addStudent(obj: Student) { // addStudent method banaya
...
const programStart = async () => { // programStart function banaya jo async hai
  do { // do-while loop start kiya
    console.log("Welcome!"); // Welcome message print kiya
    const ans = await inquirer.prompt({ // inquirer se user se prompt lete hain
    ...
    if (ans.select === "Staff") { // agar user Staff choose kare
      console.log("You approach the staff room. Please feel free to ask any questions."); // Staff room ka message print
    } else if (ans.select === "Student") { // agar user Student choose kare
      const studentAns = await inquirer.prompt({ // naya prompt lete hain user se
      ...
      const student = person.students.find((val) => val.name === studentAns.student); // students array me se matching student dhoond rahe hain

      if (!student) { // agar student nahi milta
      ...
      } else { // agar student mil jata hai
    ...
    } else if (ans.select === "Exit") { // agar user Exit choose kare
